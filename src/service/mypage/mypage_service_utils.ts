...
const get_follower = async( user_id:string ): Promise<follow_res_dto> => {
    try {
        ...
        for (const follow of follows) {
            const follows_user = await find_user_by_id(follow.from_user_id.toString());
 ...
 * 사용자가 리뷰 작성한 작품 반환
 */
const get_user_reviewed = async( user_id:string ): Promise<review_profile_res_dto> => {
    try{
 ...
 * 사용자가 찜한 작품 반환
 */
const get_user_bookmarked = async( user_id:string ): Promise<musical_profile_res_dto> => {
    try{
