...
import { review_profile_res_dto, review_profile_item_dto } from "../../dto/review/response/review_profile_res";
import { review_mypage_detail_res_dto } from "../../dto/review/response/review_mypage_detail_res"
import { musical_profile_res_dto, musical_profile_item_dto } from "../../dto/musical/response/musical_profile_res";
import { follow_item_dto, follow_res_dto } from "../../dto/follow/response/follow_res";
 ...
 * 내 프로필 반환
 */
const get_my_profile = async( user_id:string ) : Promise<user_mypage_res_dto> => {
    try{
        ...
        // 북마크한 작품 반환
        const bookmark_list = await mypage_service_utils.get_user_bookmarked(user_id);
 ...
 * 내가 작성한 리뷰 상세 모달 반환
 */
const mypage_review_detail = async( review_id: string ) : Promise<review_mypage_detail_res_dto> => {
    try{
        const review = await mypage_service_utils.find_review_by_id(review_id);
        const review_musical = await mypage_service_utils.find_musical_by_id(review.musical_id.toString());
 ...
 * 개인정보 수정
 */
const update_profile = async( user_id: string, user_update_dto: user_update_dto ): Promise<user_update_res_dto>=> {
    try{
        const user = await mypage_service_utils.find_user_by_id(user_id);

        const updated_user = await Users.findByIdAndUpdate(user._id, user_update_dto);
